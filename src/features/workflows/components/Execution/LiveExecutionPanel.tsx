...
const ToolCallItem = ({
  ...
  return (
    <div
      ...
      style={{
        borderColor: status === 'error' ? '#ef4444' : status === 'completed' ? '#10b981' : '#f59e0b',
...
export default function LiveExecutionPanel({
  ...
  const agentSections = useMemo(() => {
    ...
    for (const event of newEvents) {
      ...
      if (!agentLabel && !nodeId) {
        ...
        if (isReplay) {
          console.warn(
            `[LiveExecutionPanel] Skipping event without agent context:`,
            {
              type: event.type,
  ...
  // Instant streaming effect - show all content immediately
  useEffect(() => {
    if (!isVisible) return;

    // Show all content immediately - no artificial delays
    // In replay mode, we always want to show everything instantly
    if (totalTextLength > visibleCharCount || isReplay) {
      setVisibleCharCount(totalTextLength);
    }
  }, [isVisible, totalTextLength, visibleCharCount, isReplay]);
  ...
  }, [visibleCharCount, totalTextLength, agentSections]);
  ...
  }, [visibleCharCount, isVisible, isAutoScroll, totalTextLength, agentSections]);
  ...
  // Render text with character limit (selects raw vs clean without heavy regex each render)
  const renderTextWithLimit = (text: string, startIndex: number): { rendered: string; charsUsed: number } => {
    // In replay mode, always show everything
    if (isReplay) {
      return { rendered: text, charsUsed: text.length };
    }
    const availableChars = Math.max(0, visibleCharCount - startIndex);
    const rendered = text.substring(0, availableChars);
    return { rendered, charsUsed: text.length };
  };
