...
| --- | --- | --- |
| `langchain-middleware-compat` | Inherit the public LangChain middleware contract so default timestamp middleware works with current compatible dependency resolution. No dependency-range or retry-policy change. | Real fixture-model agent invocation plus existing factory/middleware regressions: 28 passed. |
